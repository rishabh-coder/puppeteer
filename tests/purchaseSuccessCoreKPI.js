...
const runPuppeteer = async () => {
    ...
    await page.type(loginPage.fillEmail,'aaadekk@sstk.com');
    ...
    await page.waitForFunction('dataLayer && dataLayer.find(o => o.event === "purchaseSuccess")', { timeout: 50000 });
    const purchaseSuccessCoreKPI = await page.evaluate(() => {
    const datalayerEvent = dataLayer.find(o => o.event === 'purchaseSuccess');
    return datalayerEvent;
    });
    console.log(purchaseSuccessCoreKPI.event);
    expect(purchaseSuccessCoreKPI.event).to.equal('purchaseSuccess');
